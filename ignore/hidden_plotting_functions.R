#
#
#  hidden_plotting_functions.R
#  ===========================
#
#  This is a set of hidden (not @exported) functions used by
#  function xtsplot to provide special functionality including:
#   . log plots
#   . shaded areas
#   . highlight entry / exit dates
#   . nice set of colors and format
#
#  The code is based on xtsExtra and a blog post on timelyportfolio, with
#  modifications.
#
#  http://timelyportfolio.blogspot.ca/2013/02/shading-and-points-with-xtsextra-plotxts.html
#  https://raw.github.com/timelyportfolio/plotxts_shading_points/master/plot.R
#
#
...
#
#'
#' plot_ext function (Not implemented)
plot_ext   <- function(x, y = NULL, screens = 'auto', layout.screens = 'auto', ...,
...
do_add.shading <- function(blocks, y){
  #cat("add_shading:  ylim = ", y, '\n')

  if(par("ylog")) {
    yrng <- c(1.3*max(y), 0.7*min(y))
  } else {
...
#
#' Custom panel for the custom xtsplot function (Not implemented)
#'
#' Custom method is not yet implemented.  Instead, use method = "zoo"
#' when calling xtsplot (the default)
#'
#'
custom.panel <- function(index,x,...) {
  default.panel(index,x,...)
  #print(paste("yaxp:", par("yaxp")))

  #print(par("usr"))
  abline(h=pretty(c(par("yaxp")[1],par("yaxp")[2]),n=abs(par("yaxp")[3])),col="gray60",lty=3)

  if(par("ylog") == FALSE) {  # par yaxp has different meaning in log mode, so don't draw lines
    abline(h=10^par("usr")[3], col="black")
  } else {
  ...
  # Points to highlight entry date
  points(x=index[which(index(x) %in% as.Date(buydates))],
         y=x[which(index(x) %in% as.Date(buydates)),1],cex=1,pch=19,
         col="springgreen4")

  # Points to highlight exit date
  points(x=index[which(index(x) %in% as.Date(selldates))],
         y=x[which(index(x) %in% as.Date(selldates)),1],cex=1,pch=19,
         col="indianred3")
