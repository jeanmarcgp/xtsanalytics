####################################################################################
# FILE wfo_h2o_ensemble.R
#
#
# Also, add the adjusted (no lookahead) equity curve code to the function
# and return list.
####################################################################################
# FUNCTION wfo_h2o_ensemble
#
#' Performs walk-forward optimization using the h2o ensemble function.
#'
#' Builds an h2o ensemble model at each optimization point in time for
#' a given set of parameters.  This function is multi CPU threaded and builds
#' a single model at each point in time.  The multi-threading is done by
#' the underlying function h2o.ensemble(), and it uses N-1 cores on the
#' host machine by default.
#'
#'
...
#'
#' @export
wfo_h2o_ensemble <- function(featuremat, modelwindow = 252, wfo_span = "months",
                            ylag = 21, SPwindow = 63, jobname = NA, verbose = FALSE,
                            mtry = 1, ntree = 500, importance = TRUE, earliest = NA,
                            na.action = na.omit, ...) {
  ...
  #---------------------------------------------------------
  # Specify base learners and meta learner
  #---------------------------------------------------------
  learner     <- c("h2o.glm.wrapper", "h2o.randomForest.wrapper",
                   "h2o.gbm.wrapper",   "h2o.deeplearning.1",  "h2o.deeplearning.2",
                   "h2o.deeplearning.3", "h2o.deeplearning.4" )
  ...
  res$yhat   <- NA
  res        <- xtsbind(res, emptyxts(cnames = learner, nc = length(learner),
                                      order.by = index(res)))
  ...
  #---------------------------------------------------------
  # Start the h2o cluster before the loop.
  #---------------------------------------------------------
  h2o.init(nthreads = -1)  # Start H2O cluster with nthreads = num cores on your machine
  ...
  for(i in 1:Ndates) {
    ...
    if(i == Ndates) {
      ...
      #------------------------------------------------------
      # Set up and train the h2o Ensemble model
      # Load training data into h2o cluster
      #------------------------------------------------------
      #------------------------------------------------------
      # First, remove all older models (garbage collection)
      #------------------------------------------------------
      h2o.removeAll()
      train_key <- paste0("train_key_", i)
      y         <- "y"
      x         <- setdiff(names(featuremat), y)  # all others!
      train     <- as.h2o(x = datatrain, destination_frame = train_key)
    ...
    test     <- as.h2o(x = datapred, destination_frame = test_key)

    mpred    <- predict(fit, test)

    yhat     <- as.data.frame(mpred$pred)[, 1]
    allpreds <- cbind(yhat, as.data.frame(mpred$basepred))

    #res$yhat[interval - wfo_points[1]] <- as.numeric(yhat)

    res[interval - wfo_points[1], 2:reswidth] <- as.matrix(allpreds)


  }  #####  END LOOP  #####

  #------------------------------------------------------
  # Final Garbage collection:  remove all h2o objects
  #------------------------------------------------------
  h2o.removeAll()
...
#h2o.deeplearning.2 <- function(..., hidden = c(200,200,200), activation = "RectifierWithDropout", epochs = 50, seed = 1)  h2o.deeplearning.wrapper(..., hidden = hidden, activation = activation, seed = seed)
