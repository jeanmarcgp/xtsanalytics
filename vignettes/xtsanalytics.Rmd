---
title: "Package xtsanalytics"
author: "Jean-Marc Patenaude"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{xtsanalytics package vignette and overview}
  %\VignetteEngine{knitr::rmarkdown}
  \usepackage[utf8]{inputenc}
---

The **xtsanalytics package** provides a set of function for analysis and development of investment strategies. The package includes functions in these general categories:
...
- Features and indicator generating functions, suitable for machine learning modeling
...
- A portfolio optimization set of functions to complement package **PortfolioAnalytics**


The package also leverages and extends several finance related packages.  These packages should be installed in your R installation since xtsanalytics assumes their availability:

- quantmod (itself requiring xts and zoo)
...
xtsanalytics makes extensive use of the xts data structure.  In addition, a new data structure
was created to easily create statistical equity curves of a portfolio built using Walk-Forward
optimization (WFO).  This is called the statfolio.  There may be different classes of statfolio
depending on which function was used to generate it.  For example, if function **optimize_statfolio()**
was used, then the resulting statfolio is of class "optimize.portfolio.rebalancing".

WFO optimizes an equity curve at a series of specific points in time using a rolling window.  For
example, a typical use would be to optimize monthly, at the end of each month.  In doing so, the
data window used is N days up to and including the WFO date, resulting in an equity curve up to the
current WFO date.  The following month, the same is repeated to generate the section of equity curve
from the previous WFO date up to and including the current WFO date. For example, for a 10 year equity 
curve, this would result in 10 years * 12 months = 120 WFO optimizations if done monthly.  Each of 
